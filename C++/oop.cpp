...
bool init()
{
    ...
    else
    {
        ...
        sdlWindow = SDL_CreateWindow( "ArkavQuarium", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_RESIZABLE);
        if( sdlWindow == NULL )
        {
...
void handle_input() {
    ...
    if (!clickedMouse.empty()) clickedMouse.clear();
...
const std::set<int>& get_clicked_mouse() {
    return clickedMouse;
}

const int get_clicked_mouseX() {
    return clickedMouseX;
}

const int get_clicked_mouseY() {
